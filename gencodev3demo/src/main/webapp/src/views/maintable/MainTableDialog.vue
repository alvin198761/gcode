/*主表管理,作者:唐植超,日期:2020-02-20 22:28:50*/
<template>
<div>
      <el-drawer :visible.sync="show" show-close modal-append-to-body destroy-on-close append-to-body close-on-press-escape  :wrapperClosable="false" :close-on-click-modal="false" :close-on-press-escape="false" size="60%">
          <div slot="title" style="font-size: 20px; font-weight: bold">
            {{title}}
          </div>

          <el-form :model="form" ref="form" :rules="rules" label-width="100px" style="flex: 1;">
      ...
      return {
		...
		// 长度示例 {min: 1, max: 10, message: '长度不正确', trigger: 'blur'},
				...
				{required: true, message: '请输入显示文本', trigger: 'blur'},      
												...
												a_id :[
				{required: true, message: '请输入a表主键', trigger: 'blur'},      
				],
												b_id :[
				{required: true, message: '请输入b表主键', trigger: 'blur'},      
				],
												date :[
