/*测试A管理,作者:唐植超,日期:2020-02-20 20:09:06*/
<template>
      <div>
            <el-form :inline="true">
                            ...
                            <el-form>
                                  <el-row :gutter="10">
												  ...
												  <el-form-item label="主键">{{props.row.id}}</el-form-item>
										                                        										  										  ...
										                                        										  										  <el-col :span="6">
												  <el-form-item label="提示文本">{{props.row.tip}}</el-form-item>
											  </el-col>
										                                        										  											  <el-col :span="6">
                								  ...
                								  <el-table-column prop="tip" label="提示文本"></el-table-column>
        ...
        <AaTableDialog ref="dialog" :refresh="refresh"></AaTableDialog>
  </div>
</template>
<script>
  import AaTableDialog from './AaTableDialog.vue';
  import moment from 'moment';
  export default {
      components: { AaTableDialog },
      data: function () {
      return {
        total: 0,
        page: 1,
    ...
    methods: {
        ...
        that.$postBody("/api/aaTable/queryPage", requestData).then(res => {
			  that.loading = false;
			  that.dataList = res.data.content;
			  that.total = res.data.totalElements;
			}).catch(res => {
			  that.$message.error("获取测试A列表失败：" + res);
			  that.loading = false;
			});
      },
      doAdd() {
      ...
      doDelete(row) {
        ...
        this.$confirm('你确定要删除吗?', '提示', {
          ...
          that.$post("/api/aaTable/delete", {id: row.id}).then(res => {
