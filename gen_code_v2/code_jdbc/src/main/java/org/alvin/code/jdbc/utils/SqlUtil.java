package org.alvin.code.jdbc.utils;
...
public class SqlUtil {
     ...
     * @return
     */
    public static String joiner(List<?> list, String join) {
        if (list.isEmpty()) {
            return "";
        }
     ...
     * @return
     */
    public static String joiner(Object[] arr, String join) {
        if (arr.length == 0) {
            return "";
        }
        return Arrays.asList(arr).stream().map(Object::toString).collect(Collectors.joining(join));
    }
}
