...
<script>
    ...
    export default{
        ...
        methods: {
            submitForm(formName) {
                const that = this;
                this.${dollar}refs[formName].validate((valid) => {
                    if (!valid) {
                        return;
                    }
                    ${dollar}.ajax({
                        ...
                        data: {
                            ...
                            j_password: ${dollar}.md5(that.form.j_password),

                        },
                        success: function (data) {
                            if (data.success) {
                                document.getElementById("j_password").value = ${dollar}.md5(document.getElementById("j_password").value);
                                document.getElementById("loginForm").submit();
                                return;
                            }
                            for (let p in data.msgMap) {
                                that.${dollar}message.error(data.msgMap[p]);
                                break;
                            }
                        },
                        error: function (error) {
                            that.${dollar}message.error(error);
                        }
                    });
